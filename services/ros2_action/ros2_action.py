...
class MyROS2ActionService(ROS2ActionService, Reconfigurable):
    ...
    cancel_on_reconfigure: (
        bool  # should service cancel existing actions on reconfigure (DEFAULT: False)
    )
    hold_results: (
        bool  # should service hold previous results for reporting (DEFAULT: True)
    )
    hold_last_n: int  # how many results should we hold (DEFAULT: 10)
    action_server_timeout: (
        int  # timeout (s) for waiting for action server (DEFAULT: 10)
    )
            ...
            if action["future"] == future:
                with self.lock:
                    ...
                    if not goal_handler.accepted:
                        self.logger.error(
                            f"goal {action['name']}, was not accepted, removing from current actions"
                        )
                        self.results.append({
                            "id": uuid.uuid4(),
                            "name": action["name"],
                        ...
                        return
                    self.logger.debug(
                        "goal was accepted and is executing, adding response callback"
                    )
                    action["goal_handler"].get_result_async()
                    action["goal_handler"].add_done_callback(
                        self._goal_response_done_callback
                    )
                    return
    ...
    def _goal_response_done_callback(self, future) -> None:
        ...
        for action in self.current_actions:
            if action["goal_handler"] == future:
                with self.lock:
                    self.logger.debug(
                        f"found action: {action['name']}, getting results and cleaning actions"
                    )
                    self.results.append({
                        "id": uuid.uuid4(),
                        "name": action["name"],
    ...
    def reconfigure(
        ...
        if self.ros_node is not None:
            if len(self.current_actions) > 0 and self.cancel_on_reconfigure:
                for action in self.current_actions:
                    self.logger.info(
                        f"stopping action {action['name']} and removing from current actions"
                    )
        ...
        for action in self.actions:
            ...
            try:
                ...
                self.logger.error(
                    f"problem importing module for {action['name']}, type failed: {action['type']}"
                )
        ...
        for action in self.actions:
            if action["name"] == goal_name:
                ...
                action_client = ActionClient(
                    self.ros_node, action["class"], action["name"]
                )
                result = action_client.wait_for_server(
                    timeout_sec=self.action_server_timeout
                )
                if not result:
                    self.logger.error(
                        f"ROS action server timeout({self.action_server_timeout}) exceeded"
                    )
                    self.logger.error(f"cannot continue")
                    return ActionResponse(
                        response=f"action server timeout: {goal_name} cannot continue"
                    )
        ...
        for action in self.current_actions:
            if action["name"] == goal_name and "goal_handler" in action:
                with self.lock:
                    ...
                    action["goal_handler"].addadd_done_callback(
                        self._goal_response_done_callback
                    )
